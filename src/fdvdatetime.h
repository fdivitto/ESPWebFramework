...
struct DateTime {
  ...
  DateTime()
      : seconds(0), minutes(0), hours(0), year(2000), month(1), day(1), timezoneHours(s_defaultTimezoneHours),
  ...
  DateTime(uint8_t day_, uint8_t month_, uint16_t year_, uint8_t hours_, uint8_t minutes_, uint8_t seconds_)
      : seconds(seconds_), minutes(minutes_), hours(hours_), year(year_), month(month_), day(day_),
        timezoneHours(s_defaultTimezoneHours), timezoneMinutes(s_defaultTimezoneMinutes) {}

  DateTime(uint8_t day_, uint8_t month_, uint16_t year_, uint8_t hours_, uint8_t minutes_, uint8_t seconds_,
           int8_t timezoneHours_, uint8_t timezoneMinutes_)
      : seconds(seconds_), minutes(minutes_), hours(hours_), year(year_), month(month_), day(day_),
        timezoneHours(timezoneHours_), timezoneMinutes(timezoneMinutes_) {}
...
private:
  ...
  static DateTime lastSyncDateTime(bool set = false,
                                   DateTime const &value = DateTime()); // datetime at the time of NTP synchronization
