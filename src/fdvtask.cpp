...
void MTD_FLASHMEM Task::setStackDepth(uint16_t stackDepth) { m_stackDepth = stackDepth; }
...
xTaskHandle MTD_FLASHMEM Task::getCurrentTaskHandle() { return xTaskGetCurrentTaskHandle(); }
...
void STC_FLASHMEM Task::delay(uint32_t ms) { vTaskDelay(ms / portTICK_RATE_MS); }

// task min touched free stack (in bytes)
uint32_t STC_FLASHMEM Task::getMinFreeStack() { return uxTaskGetStackHighWaterMark(NULL) * 4; }

// global free heap (in bytes)
uint32_t STC_FLASHMEM Task::getFreeHeap() { return system_get_free_heap_size(); }
