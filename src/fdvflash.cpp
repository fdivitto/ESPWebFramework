...
char FUNC_FLASHMEM getChar(char const *str) { return isStoredInFlash(str) ? (char)getByte(str) : *str; }
...
MTD_FLASHMEM FlashWriter::FlashWriter(void const *dest) : m_currentPage(NULL), m_maxPosition(getBeginOfSDKSettings()) {
...
void ICACHE_FLASH_ATTR FlashWriter::loadPage() {
  ...
  if (alignedDest != m_currentPage) {
    ...
    void const *addr =
        FLASH_MAP_START_PTR +
        ((uint32_t)m_currentPage & 0xFFFFF); // calculate actual address (only first megabyte is directly addressable)
